...
public extension NetworkObserver {
  /// Default implementation of `NetworkObserver` which does not use execute a perodic ``NetworkPing``.
  convenience init() where MonitorType == NWPathMonitor, PingType == NeverPing {
    let monitor = NWPathMonitor()
    self.init(monitor: monitor)
  }
  
  /// Default implementation of `NetworkObserver` which does use execute a perodic ``NetworkPing``.
