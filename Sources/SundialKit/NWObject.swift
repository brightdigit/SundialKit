  ...
  public class NWObject<MonitorType: NWPathMonitorable, PingType: NetworkPing> {
    internal init(monitor: MonitorType, ping: PingType?) {
    ...
    var pathStatusPublisher: AnyPublisher<NWPathStatus, Never> {
    ...
    var isConstrainedPublisher: AnyPublisher<Bool, Never> {
      isConstrainedSubject.eraseToAnyPublisher()
    }

    var pingStatusPublisher: AnyPublisher<PingType.StatusType?, Never> {
