...
final class CombineLatencyDashboardViewModel {
  ...
  private func setupSubscriptions() {
    ...
    // Create message decoder for typed message decoding
    ...
    connectivityObserver.messageReceived
      ...
      .compactMap { result -> Sundial_Demo_LatencyTestReply? in
        try? decoder.decode(result.message) as? Sundial_Demo_LatencyTestReply
      }
      .sink { [weak self] reply in
        self?.handleLatencyReply(reply)
      }
  ...
  func startLatencyTest() {
    ...
    pingTimer = Timer.publish(every: 2.0, on: .main, in: .common)
      .autoconnect()
      .sink { [weak self] _ in
        Task { @MainActor in
          try? await self?.sendLatencyPing()
        }
      }
  }
